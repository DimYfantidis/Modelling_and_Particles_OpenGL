...
static surface worldSurfaces[2];
static size_t n_surfaces;

// Checks if the particle p is directly above the surface S
static bool areVerticallyAligned(const surface* S, const particle* p)
{
...
static bool collisionDetection(const surface* S, const particle* p)
{
...
static inline void collisionResolution(const surface* S, particle* p)
{
...
static void castShadow(const surface* S, const particle* p)
{
...
static HANDLE particleGeneratorThread;
// ID of the worker thread
static DWORD particleGeneratorId;
// Mutex for restricting write access to the particle list -- Up to one writer allowed
static HANDLE particleListGuard;
...
static DWORD WINAPI particleGeneratorWorker(LPVOID lpParam)
{
	while (true)
	{
...
static void display(void)
{
	...
	int i = 1;
	// Log system's state
	snprintf(screenLogBuffer, sizeof(screenLogBuffer) / sizeof(char), glGetString(GL_VERSION));
	renderString2f(3.0f, (i++) * 15.0f, GLUT_BITMAP_9_BY_15, screenLogBuffer, 1.0f, 1.0f, 1.0f);
...
static void cleanupFunc(void)
{
	...
	printf("\t> Destroying window system 2\n");

	glutDestroyMenu(window_menu_id);
	glutDestroyWindow(window_id);
}

static void tickRedisplay(int value)
{
	if (PROGRAM_CONTROL_VAR != 2) {
		return;
	}
	// Draws the next frame according to the current framerate
	glutPostRedisplay();
	glutTimerFunc((int)(1000.0 * deltaTime), tickRedisplay, 0);
}

static void main_menu(int op_id)
{
	switch (op_id)
	{
	case 1:
...
int main2(int argc, char* argv[])
{
	...
	window_id = glutCreateWindow("Particle System -- (Dimitrios Yfantidis - 3938 & Evangelos Spatharis - 3949)");
